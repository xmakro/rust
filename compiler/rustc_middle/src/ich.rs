...
impl<'a> StableHashCtxt for StableHashState<'a> {
    ...
    fn assert_default_stable_hash_controls(&self, msg: &str) {
        ...
        // `hash_spans` must match the session-wide default from `StableHashState::new`, so the
        // per-session `ExpnData` hash caches stay consistent. It can be overridden locally via
        // `while_hashing_spans` (e.g. for name mangling), but that must not be used for metadata.
        assert_eq!(
