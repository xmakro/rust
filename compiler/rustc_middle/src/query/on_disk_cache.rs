...
// Vectors of pairs and maps have the same encoding. Decode the indexes
// directly into maps, while encoding the collected vectors and borrowed
// syntax-context maps without copying them.
#[derive(Encodable, Decodable)]
struct Footer<
    I = FxHashMap<SerializedDepNodeIndex, AbsoluteBytePos>,
    S = FxHashMap<u32, AbsoluteBytePos>,
> {
    file_index_to_stable_id: FxHashMap<SourceFileIndex, EncodedSourceFileId>,
    query_values_index: I,
    side_effects_index: I,
    ...
    syntax_context_tables: Vec<(u64, S)>,
...
impl OnDiskCache {
    ...
    pub fn new(sess: &Session, data: Mmap, start_pos: usize) -> Result<Self, ()> {
        ...
        Ok(Self {
            ...
            query_values_index: footer.query_values_index,
            side_effects_index: footer.side_effects_index,
    ...
    pub fn can_carry_forward(&self, new_start_pos: usize) -> bool {
        const MAX_CARRIED_GENERATIONS: usize = 8;
        cfg!(unix)
            && self.start_pos == new_start_pos
            && self.footer_pos > self.start_pos
            && self.syntax_context_tables.len() < MAX_CARRIED_GENERATIONS
            && self
                .serialized_data
                .read()
                .as_ref()
                .is_some_and(|data| data.len() >= self.footer_pos)
    }
    ...
    pub fn serialize(
        ...
        tcx.dep_graph.with_ignore(|| {
            ...
            let carried = carried_data.as_deref().map(|data| {
                assert_eq!(on_disk_cache.start_pos, encoder.position());
                &data[on_disk_cache.start_pos..on_disk_cache.footer_pos]
            });
            ...
            let (file_to_file_index, file_index_to_stable_id) = {
                ...
                if carried.is_some() {
                    ...
                    let mut stable_id_to_index: FxHashMap<
                        EncodedSourceFileId,
                        SourceFileIndex,
            ...
            let mut encoder = CacheEncoder {
                ...
                query_values_index: Default::default(),
                carried_values: GrowableBitSet::new_empty(),
                side_effects_index: Default::default(),
            };

            // Reference green values at their old positions, including values
            // loaded during this session. The encoder skips these entries;
            // red nodes and values missing from the previous cache are encoded below.
            if carried.is_some() {
                tcx.dep_graph.for_each_green_prev_index(&mut |prev_index, current_index| {
                    if let Some(&pos) = on_disk_cache.query_values_index.get(&prev_index) {
                        encoder.carried_values.insert(current_index);
...
pub struct CacheEncoder<'a, 'tcx> {
    ...
    carried_values: GrowableBitSet<DepNodeIndex>,
...
impl<'a, 'tcx> CacheEncoder<'a, 'tcx> {
    ...
    pub fn is_carried_query_value(&self, index: DepNodeIndex) -> bool {
        self.carried_values.contains(index)
    }
...
mod tests {
    ...
    #[test]
    fn footer_borrowed_encoding_and_direct_map_decoding() {
        let index = SerializedDepNodeIndex::from_u32(12);
        let mut contexts = FxHashMap::default();
        contexts.insert(2, AbsoluteBytePos::new(91));
        contexts.insert(7, AbsoluteBytePos::new(1234));
        let borrowed = Footer {
            file_index_to_stable_id: FxHashMap::default(),
            // Repeated entries exercise the existing last-entry-wins behavior.
            query_values_index: vec![
                (index, AbsoluteBytePos::new(42)),
                (index, AbsoluteBytePos::new(84)),
            ],
            side_effects_index: vec![(index, AbsoluteBytePos::new(126))],
            interpret_alloc_index: vec![256, 512],
            syntax_context_tables: vec![(4096, &contexts)],
            expn_data: UnhashMap::default(),
            foreign_expn_data: UnhashMap::default(),
        };
        let owned = Footer {
            file_index_to_stable_id: FxHashMap::default(),
            query_values_index: borrowed.query_values_index.clone(),
            side_effects_index: borrowed.side_effects_index.clone(),
            interpret_alloc_index: borrowed.interpret_alloc_index.clone(),
            syntax_context_tables: vec![(4096, contexts.clone())],
            expn_data: UnhashMap::default(),
            foreign_expn_data: UnhashMap::default(),
        };
        let mut encoder = MemEncoder::new();
        borrowed.encode(&mut encoder);
        let mut bytes = encoder.finish();
        let mut encoder = MemEncoder::new();
        owned.encode(&mut encoder);
        assert_eq!(bytes, encoder.finish());
        bytes.extend_from_slice(rustc_serialize::opaque::MAGIC_END_BYTES);
        let mut decoder = MemDecoder::new(&bytes, 0).unwrap();
        let decoded: Footer = Decodable::decode(&mut decoder);
        assert_eq!(decoded.query_values_index.len(), 1);
        assert_eq!(decoded.query_values_index[&index], AbsoluteBytePos::new(84));
        assert_eq!(decoded.side_effects_index[&index], AbsoluteBytePos::new(126));
        assert_eq!(decoded.interpret_alloc_index, [256, 512]);
        assert_eq!(decoded.syntax_context_tables, [(4096, contexts)]);
