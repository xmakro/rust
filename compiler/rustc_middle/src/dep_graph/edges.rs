...
/// Records the reads of a task with many reads, deduplicated and in first-read order.
/// Pooled globally, so the read list and epoch filter are reused across tasks.
