...
fn load_dep_graph(sess: &Session) -> LoadResult {
    ...
    match file_format::open_incremental_file(sess, &path) {
        ...
        Ok(OpenFile { mmap, start_pos }) => {
            ...
            // Retain the file bytes so that the record region can be carried into the
            // next session's dep-graph file wholesale. The decoder borrows `mmap`, so
            // release it before handing ownership to the freshly decoded (uniquely
            // owned) graph.
            drop(decoder);
