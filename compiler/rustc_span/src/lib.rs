...
/// the current session's `source_span` value on decode. Consumers that render the position
/// into a cached artifact (line numbers in `#[track_caller]`, debuginfo, coverage) depend on
/// `def_anchor` instead, which hashes the rendered line index and column of the definition's
/// start along with its extent's line structure; see `TyCtxt::track_def_anchor`.
