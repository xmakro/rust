...
impl<'gcc, 'tcx> CodegenCx<'gcc, 'tcx> {
    ...
    pub fn lookup_debug_loc(&self, pos: BytePos) -> DebugLoc {
        // The lookup notifies `LINE_TABLE_TRACK`; the anchors recorded per function and
        // per span parent in `rustc_codegen_ssa::mir::debuginfo` cover it, and an
        ...
        let (line, col) = match line_index {
            Some(line_index) => {
                let line_pos = file.lines_untracked()[line_index];
