...
pub(crate) fn promote_from_disk_inner<'tcx, C: QueryCache>(
    ...
    // Load the disk-cached value into memory directly. The node is already
    // green, all queries have completed, and promotion runs on a single
    // thread, so the bookkeeping performed by normal query execution (job
    // registration, re-marking the node green) can be skipped.
    let dep_graph_data =
