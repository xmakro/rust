...
// functions re-fingerprints; the explicit `def_position` dependency recorded by the
// line-rendering consumers is the only thing that invalidates their cached object code.
// Without it, the second build would replay stale `#[track_caller]` lines and stale
// `DW_AT_decl_line`s from the incremental cache.
